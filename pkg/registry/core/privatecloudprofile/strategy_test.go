...
import (
	...
	privatecloudprofileregistry "github.com/gardener/gardener/pkg/registry/core/privatecloudprofile"
)

var _ = Describe("PrepareForCreate", func() {
	var privateCloudProfile *core.PrivateCloudProfile

	It("should drop the expired Kubernetes and MachineImage versions from the privatecloudprofile", func() {
		var (
		...
		privatecloudprofileregistry.Strategy.PrepareForCreate(context.TODO(), privateCloudProfile)

		Expect(privateCloudProfile.Spec.Kubernetes.Versions).To(ConsistOf(
		...
		Expect(privateCloudProfile.Spec.MachineImages).To(ConsistOf(
			MatchFields(IgnoreExtras, Fields{
