...
var (
	_          = admissioninitializer.WantsInternalCoreInformerFactory(&ValidateNamespacedCloudProfile{})
...
func (v *ValidateNamespacedCloudProfile) SetInternalCoreInformerFactory(f gardencoreinformers.SharedInformerFactory) {
...
func (c *validationContext) validateMachineTypes(a admission.Attributes) error {
	...
	parentMachineTypesLoop:
		for _, parentMachineType := range c.parentCloudProfile.Spec.MachineTypes {
			...
			if a.GetOperation() == admission.Update {
				for _, oldMachineType := range c.oldNamespacedCloudProfile.Spec.MachineTypes {
